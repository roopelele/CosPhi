            ...
            if (examineSegment(image, &outer, ii, outerAreaRatio)) {
                ...
                if (buffer[pos] == -1) {
                    if (examineSegment(image, &inner, pos, innerAreaRatio)) {
                        ...
                        if (debug > 5)
                            printf("Area ratio should be %.3f, but is %.3f, that is %.0f%% off. ", areasRatio, (float)outer.size / inner.size, (1 - outer.size / areasRatio / inner.size) * 100);
                        if ((float)outer.size / areasRatio / (float)inner.size - ratioTolerance < 1.0 && (float)outer.size / areasRatio / (float)inner.size + ratioTolerance > 1.0) {
                            ...
                            if (debug > 5)
                                fprintf(stdout, "Concentricity %.0f %.0f %.0f %.0f.", inner.x, inner.y, outer.x, outer.y);
...

                            {
                                ...
                                if (circularity - 1.0 < circularityTolerance && circularity - 1.0 > -circularityTolerance) {
                                    ...
                                    if (debug > 5)
                                        printf("Angle: %.3f %.3f \n", outer.angle, orient);
                                    if (fabs(normalizeAngle(outer.angle - orient)) > M_PI / 2)
                                        outer.angle = normalizeAngle(outer.angle + M_PI);
