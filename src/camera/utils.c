...
static int fillbits(in, le, bi) struct in* in;
int le;
unsigned int bi;
{
    int b, m;
...
static int dec_readmarker(in) struct in* in;
{
    int m;
...
static int dec_rec2(in, hu, runp, c, i) struct in* in;
...
static void decode_mcus(in, dct, n, sc, maxp) struct in* in;
int* dct;
int n;
struct scan* sc;
int* maxp;
...
static void dec_makehuff(hu, hufflen, huffvals) struct dec_hufftbl* hu;
int* hufflen;
...
static void idctqtab(qin, qout) unsigned char* qin;
PREC* qout;
{
    int i, j;
...
getPictureName(char* Picture, int fmt)
{
    ...
    sprintf(temp, "P-%02d:%02d:%04d-%02d:%02d:%02d.%s",
...
int get_pictureYV2(unsigned char* buf, int width, int height)
{
    ...
    if (picture) {
        Pyuv422torgb24(buf, picture, width, height, 0);
    }
    else {
