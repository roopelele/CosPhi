...
int CCamera::init(const char* deviceName, int* wi, int* he, bool saveI)
{
    ...
    if (strncmp(deviceName, "/dev/", 5) == 0)
        cameraType = CT_WEBCAM;
    else
        cameraType = CT_FILELOADER;
    if (strncmp(&deviceName[strlen(deviceName) - 4], ".avi", 4) == 0)
        cameraType = CT_VIDEOLOADER;
...
int CCamera::renewImage(CRawImage* image, bool move)
{
    if (cameraType == CT_WEBCAM) {
        ...
        Pyuv422torgb24(videoIn->framebuffer, image->data, videoIn->width, videoIn->height, USE_STEREO_CAMERA ? 1 : 0);
    ...
    if (cameraType == CT_VIDEOLOADER) {
        if (readNextFrame) {
            ...
            if (jpeg_decode(&aviBuffer2, (unsigned char*)aviBuffer1, &width, &height) < 0) {
                ...
                Pyuv422torgb24(aviBuffer2, image->data, width, height, 0);
                return 0;
            }
        }
        else {
